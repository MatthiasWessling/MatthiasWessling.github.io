...
date = '2026-08-07'
draft = false
summary = 'From learning polymer permeability and layer-by-layer design to embodying memory in PEDOT:PSS—neural networks as surrogates for transport, then as physical substrates whose ionic–electronic state retains and transforms history.'
abstract = 'From learning polymer permeability and layer-by-layer design to embodying memory in PEDOT:PSS—neural networks as surrogates for transport, then as physical substrates whose ionic–electronic state retains and transforms history.'
...
tags = ['neural networks', 'machine learning', 'membranes', 'neuromorphic', 'physical memory', 'mixed ionic-electronic']
+++

This note summarizes how artificial neural networks have been used in our group, from early structure–property correlation to membrane–process design and, later, to organic neuromorphic hardware. The arc moves from *predicting* transport in soft matter to *embodying* memory and nonlinear transformation in permeable mixed ionic–electronic materials—where ion distribution, oxidation and self-discharge form a hidden state that retains recent input.

## Polymer permeability from infrared spectra (1994)

In *[Modelling the permeability of polymers: a neural network approach](https://doi.org/10.1016/0376-7388(93)E0168-J)* (*Journal of Membrane Science*, 1994), we examined whether a neural network can map the infrared spectrum of a polymer to its carbon dioxide permeability. The IR spectrum was treated as a high-dimensional input; permeability was the output. The underlying assumption was that sufficient chemical information is encoded in the spectrum for a data-driven mapping, without first writing a full free-volume or solution–diffusion model. The best network investigated showed predictive capability. The work is short and cautious in its claims; its main lasting point is methodological: non-linear membrane–material relationships can be learned from spectroscopic and transport data—capacity as a measured mapping between material coordinates and transport response.

## Data-driven design of layer-by-layer membranes (Rall)

Deniz Rall’s thesis, *[Data-driven development of layer-by-layer nanofiltration membranes and processes](https://publications.rwth-aachen.de/record/804603)*, shifted the question from predicting properties of existing polymers to navigating the fabrication space of layer-by-layer (LbL) ion-separation membranes and embedding that description in process optimization. Fixed-charge multilayers are themselves architected soft matter; the design task is to connect synthesis protocol to selective ionic transport.
...
In this phase, neural networks function as surrogates and as components of optimization workflows that connect laboratory synthesis protocols to process performance—forward maps from architecture and chemistry to transport capacity, with the physics retained where it can be identified.
...
Across these stages, the common element is the coupling of transport and soft-matter physics to learning—whether the network predicts membrane properties, guides fabrication and plant design, or is itself realized as a wet electrochemical device whose constitutive ionic–electronic state retains and transforms recent history.
